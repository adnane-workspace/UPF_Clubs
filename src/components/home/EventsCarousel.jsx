...
const Slide = memo(({ ev, direction, variants, onShowDetails, onNext }) => (
  ...
  >
    <img 
      ...
      loading="lazy"
    />
    <div className="absolute inset-0 bg-gradient-to-t from-[var(--bg-primary)] via-[var(--bg-primary)]/70 to-transparent transition-colors duration-300" />
    ...
    <div className="relative z-30 h-full flex flex-col justify-end pb-32 sm:pb-28 lg:pb-24 px-4 sm:px-8 lg:px-16 max-w-7xl mx-auto w-full">
      <div className="max-w-2xl">
        ...
        <h1 className="text-3xl sm:text-5xl lg:text-6xl font-black text-[var(--text-primary)] transition-colors duration-300 leading-tight mb-3 sm:mb-4">
          {ev.title}
        </h1>
        <p className="text-[var(--text-secondary)] transition-colors duration-300 text-sm sm:text-base lg:text-lg mb-6 sm:mb-8 max-w-xl">
          {ev.description}
        </p>
        <div className="flex flex-wrap gap-3 sm:gap-4">
          ...
          <button
            type="button"
            onClick={onNext}
            className="bg-[var(--bg-tertiary)] hover:bg-[var(--bg-secondary)] border border-[var(--border-color)] text-[var(--text-primary)] font-bold px-5 sm:px-6 py-3 rounded-xl transition-all flex items-center gap-2 min-h-[44px]"
          >
...
export default function EventsCarousel() {
  ...
  return (
    <section 
      className="relative w-full min-h-[92svh] sm:min-h-[86vh] lg:h-[80vh] overflow-hidden bg-[var(--bg-primary)] transition-colors duration-300"
      style={{
    ...
    >
      ...
      <div className="absolute bottom-4 sm:bottom-8 left-0 right-0 z-10 px-4 sm:px-8 lg:px-16 max-w-7xl mx-auto flex flex-col sm:flex-row items-start sm:items-center gap-4 sm:gap-0 justify-between pointer-events-none">
        <div className="flex gap-3 pointer-events-auto">
          {eventsData.map((_, i) => (
            <button
              ...
              className="relative h-1 w-7 sm:w-10 bg-[var(--bg-tertiary)] overflow-hidden rounded-full"
            >
              <motion.div 
        ...
        <div className="flex gap-2 pointer-events-auto">
          <button onClick={prev} className="w-9 h-9 sm:w-10 sm:h-10 rounded-full border border-[var(--border-color)] bg-[var(--bg-secondary)] flex items-center justify-center text-[var(--text-primary)] hover:bg-[var(--bg-tertiary)] transition-colors">
            <IconChevronLeft size={18} />
          </button>
          <button onClick={next} className="w-9 h-9 sm:w-10 sm:h-10 rounded-full border border-[var(--border-color)] bg-[var(--bg-secondary)] flex items-center justify-center text-[var(--text-primary)] hover:bg-[var(--bg-tertiary)] transition-colors">
